...
open class GridList<T> {
    ...
    constructor(width: Int, height: Int, init: (Int, Int) -> T) : this(width, height, { p: Pair<Int, Int> -> init(p.first, p.second) })
    ...
    fun getOrElse(x: Int, y: Int, defaultValue: (Int, Int) -> T): T = getOrElse(Pair(x, y), { p -> defaultValue(p.first, p.second) })
